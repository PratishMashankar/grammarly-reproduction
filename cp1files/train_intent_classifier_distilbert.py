...
import numpy as np
from transformers import Trainer, TrainingArguments, DistilBertTokenizer, DistilBertModel, DistilBertConfig, DistilBertForSequenceClassification
...
def main(args):
    ...
    tokenizer_cache_dir= 'distilbert-base-uncased-tokenizer-cache/'
    model_cache_dir= 'distilbert-base-uncased-model-cache/'
    model_type = DistilBertModel
    config_type = DistilBertConfig
...
class CustomModel(nn.Module):
    def __init__(self, args, model_class, model_name_or_path, from_tf=False, config=None, cache_dir=None, pos_weight=None):
        ...
        self.config = config
        self.distilbert = model_class.from_pretrained(model_name_or_path, from_tf=from_tf, config=config, cache_dir=cache_dir)
