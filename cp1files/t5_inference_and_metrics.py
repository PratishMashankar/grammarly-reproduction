...
def get_pred(model, tokenizer, text):
    ...
    inputs.to(device)
    outputs = model.generate(inputs["input_ids"], attention_mask=inputs["attention_mask"], max_length=512, early_stopping=True)
...
def main(args):
    ...
    for ins in tqdm(sources):
        preds.append(get_pred(model, tokenizer, ins))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', '-c', required=True,
                        help='path to T5 model checkpoint')
